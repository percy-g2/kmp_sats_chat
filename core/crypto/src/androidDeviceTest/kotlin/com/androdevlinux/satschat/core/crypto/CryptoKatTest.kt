...
/**
 * Known-Answer Tests for the Android libsodium primitives, run on a device/emulator
 * (:core:crypto:connectedAndroidTest). Vectors are from the RFCs / xchacha draft — see comments.
 * Any change to a primitive must keep these green (per-platform crypto gate).
 */
...
class CryptoKatTest {
    ...
    @Test
    fun x25519_publicKey_and_shared_secret_agree() {
        val (_, aSecretPk) = crypto.ed25519Keypair() // just to exercise randomness path
        require(aSecretPk.isNotEmpty())
    ...
    fun aead_xchacha20poly1305_ietf_draft_vector() {
        ...
        // Tamper → authentication must fail (null), never leak plaintext.
