...
/** Double Ratchet interop tests on a device/emulator (real libsodium). Mirrored in iosTest. */
...
class DoubleRatchetTest {
    ...
    fun skippedMessageAcrossDhRatchet() {
        ...
        val a1 = alice.encrypt("a1".encodeToByteArray(), ad) // delivered LATE, after Alice ratchets
        ...
        val a2 = alice.encrypt("a2".encodeToByteArray(), ad) // new sending chain
        assertEquals("a2", bob.decrypt(a2, ad).decodeToString())
        assertEquals("a1", bob.decrypt(a1, ad).decodeToString()) // skipped key from the previous chain
    }
